...
def main(identifier, language, outfile):
    with Session() as session:
        ...
        params = (
            session.query(
                T141_Block, T150_BlockValue, T141_Block_2, T150_BlockValue_2
            ).join(
                T144_BlockChild, T144_BlockChild.fkT141_Block_Child == T141_Block.id
            ).join(
                T150_BlockValue, T150_BlockValue.fkT141_Block == T141_Block.id
            ).join(
                T144_BlockChild_2, T144_BlockChild_2.fkT141_Block_Parent == T141_Block.id
            ).join(
                T141_Block_2, T141_Block_2.id == T144_BlockChild_2.fkT141_Block_Child
            ).join(
                T150_BlockValue_2, T150_BlockValue_2.fkT141_Block == T141_Block_2.id
            ).join(
                ...
                T144_BlockChild.fkT141_Block_Parent,
                T144_BlockChild.fkT141_Block_Child
            )
        ...
        for block, value, block2, value2 in params:
            _params.append({
                ...
                "name2": block2.name,
                "text": "".join(e.data for e in block2.text.data if e.language.identifier == language),
                "scaling": value2.ppe_scaling.definition,
                "unit": "".join(e.data for e in value2.ppe_text_unit.data if e.language.identifier == language),
            })
